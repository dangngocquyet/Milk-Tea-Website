...
<body>
  ...
  <section class="cart_area padding_top1">
    <div class="container">
                    ...
                    <a href="history.php" class="btn_1" style="float:right; margin-bottom:20px;">Lịch sử đặt hàng</a>
                        ...
                        <?php
                            ...
                            if (rowCount("SELECT * FROM donhang WHERE id_taikhoan=$idtaikhoan && status=0") > 0) {
                                ...
                                if (rowCount("SELECT * FROM chitiet WHERE id_donhang=$idDh") > 0) {
                        ?>
                        <table class="table">
                            <thead>
                            <tr>
                            ...
                            <?php
                                $tongcong =0;
                                foreach (selectAll("SELECT * FROM chitiet WHERE id_donhang=$idDh") as $item) {
                            ...
                            <tr>
                                ...
                                <div class="product_count">
                                    <input class="input-number" type="number" 
                                           name="soluong[<?= $item['id_sanpham'] ?>]" 
                                           value="<?= $item['soluong'] ?>" 
                                           min="1" max="100"
                                           onchange="updatePrice(this, <?= $item['gia'] ?>, <?= $item['id_sanpham'] ?>)"/>
                                </div>
                                </td>
                                <td>
                                <h5 id="total_<?= $item['id_sanpham'] ?>"><?= number_format($tong) ?>đ</h5>
                                </td>
                                <td>
        ...
        if (isset($_GET['removeproduct'])) {
            selectAll("DELETE FROM chitiet WHERE id_donhang=$idDh && id_sanpham={$_GET['removeproduct']}");
  ...
  function updatePrice(input, price, productId) {
      ...
      document.getElementById('total_' + productId).innerHTML = total.toLocaleString() + 'đ';
      
      // Tính lại tổng cộng
      let allTotals = document.querySelectorAll('[id^="total_"]');
      let grandTotal = 0;
      allTotals.forEach(element => {
          let value = parseInt(element.innerHTML.replace(/[^\d]/g, ''));
          grandTotal += value;
      });
      
      // Cập nhật tổng cộng
      document.querySelector('h5:contains("Tổng cộng")').nextElementSibling.innerHTML = grandTotal.toLocaleString() + 'đ';
  }
  </script>
</body>
