...
if (isset($_POST['add_to_cart'])) {
    ...
    if (isset($_COOKIE["user"])) {
        ...
        // Lấy giá sản phẩm
        foreach (selectAll("SELECT gia FROM sanpham WHERE id=$idSanpham") as $row) {
            $giatien = $row['gia'];
        }
        ...
        if (rowCount("SELECT * FROM donhang WHERE status=0 && id_taikhoan=$id_nguoidung") > 0) {
            ...
            if (rowCount("SELECT * FROM chitiet WHERE id_donhang = $idDhcu && id_sanpham = $idSanpham") > 0) {
                selectAll("UPDATE chitiet SET soluong = soluong + 1 WHERE id_donhang = $idDhcu && id_sanpham = $idSanpham");
            } else {
                                ...
                                <div class="col-lg-4 col-sm-6" style="height: 500px;">
                                    <div class="single_product_item text-center">
                                        <a href="detail.php?id=<?= $row['id'] ?>">
                                            <img src="img/product/<?= $row['anh1'] ?>" 
                                                 style="width: 230px; height: 230px; margin: 0 auto; display: block;" alt="">
                                        </a>
                                        <div class="single_product_text">
                                            <h4 style="font-size: 16px; margin: 15px 0;"><?= $row['ten'] ?></h4>
                                            <h3 style="margin: 10px 0;"><?= number_format($row['gia']) . 'đ' ?></h3>
                                            <div class="product_actions">
                                                <form method="POST" style="display: inline;">
                                                    <input type="hidden" name="product_id" value="<?= $row['id'] ?>">
                                                    <button type="submit" name="add_to_cart" class="add_cart">+ Thêm vào giỏ</button>
                                                </form>
                                            </div>
                                        </div>
                                    </div>
